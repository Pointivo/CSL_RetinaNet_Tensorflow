...
from data.io.convert_data_to_tfrecord import read_xml_gtbox_and_label


def eval_with_plac(img_dir, det_net, image_ext, test_annotation_path, draw_imgs=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        for a_img_name in pbar:
            ...
            if draw_imgs:
                ...
                det_detections_r = draw_box_in_img.draw_boxes(raw_img,
                ...
                save_dir = os.path.join('test_hrsc2016', cfgs.VERSION, 'hrsc2016_img_vis')
                tools.mkdir(save_dir)

                gt_xml = os.path.join(test_annotation_path, '{}.xml'.format(a_img_name))
                _, _, gtboxes_and_labels = read_xml_gtbox_and_label(gt_xml)
                gtboxes_and_labels = backward_convert(gtboxes_and_labels)
                if gtboxes_and_labels.shape[0] != 0:
                    det_detections_r = draw_box_in_img.draw_boxes(det_detections_r,
                                                                  boxes=gtboxes_and_labels[:, :-1],
                                                                  labels=gtboxes_and_labels[:, -1],
                                                                  scores=np.ones_like(gtboxes_and_labels[:, -1]),
                                                                  color=(0, 0, 255),
                                                                  method=1,
                                                                  in_graph=True)
...
if __name__ == '__main__':
    ...
    eval(args.img_dir, args.image_ext, args.test_annotation_path, args.draw_imgs)
