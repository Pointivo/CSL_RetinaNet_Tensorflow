    ...
    with tf.Session(config=config) as sess:
        ...
        for a_img in images:
            ...
            for short_size in img_short_side_len_list:
                ...
                resized_img, detection_boxes_angle, detected_scores, detected_categories = \
                    sess.run(
...
def test_icdar2015(det_net, real_test_img_list, gpu_ids, show_box):
    ...
    for i in range(nr_records):
        ...
        if show_box:
            ...
            tools.mkdir(os.path.join(save_path, 'mlt_img_vis'))
            ...
            tools.mkdir(os.path.join(save_path, 'mlt_res'))
            fw_txt_dt = open(os.path.join(save_path, 'mlt_res', 'res_{}.txt'.format(res['image_id'].split('/')[-1].split('.')[0])), 'w')
