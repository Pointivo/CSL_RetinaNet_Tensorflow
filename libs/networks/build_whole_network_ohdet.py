...
class DetectionNetwork(object):
    ...
    def rpn_reg_net(self, inputs, scope_list, reuse_flag, level):
        ...
        rpn_head_cls = slim.conv2d(rpn_conv2d_3x3,
    ...
    def rpn_net(self, feature_pyramid):
        ...
        with tf.variable_scope('rpn_net'):
            with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(cfgs.WEIGHT_DECAY)):
                ...
                rpn_head_cls = tf.concat(rpn_head_cls_list, axis=0)
                rpn_angle_cls = tf.concat(rpn_angle_cls_list, axis=0)

            return rpn_all_delta_boxes, rpn_all_boxes_scores, rpn_all_boxes_probs, rpn_head_cls, rpn_angle_cls
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch_h, gtboxes_batch_r, gthead_quadrant,
        ...
        # 2. build rpn
        rpn_box_pred, rpn_cls_score, rpn_cls_prob, rpn_head_cls, rpn_angle_cls = self.rpn_net(feature_pyramid)
        ...
        if self.is_training:
            with tf.variable_scope('build_loss'):
                labels, target_delta, anchor_states, target_boxes, target_head_quadrant, target_smooth_label = tf.py_func(
                    ...
                    inp=[gtboxes_batch_h, gtboxes_batch_r, gthead_quadrant,
                ...
                if cfgs.DATASET_NAME.startswith('DOTA'):
                    head_cls_loss = losses.head_specific_cls_focal_loss(target_head_quadrant, rpn_head_cls,
                                                                        anchor_states, labels,
                                                                        specific_cls=[6, 7, 8, 9, 10, 11])
                else:
                    head_cls_loss = losses.head_focal_loss(target_head_quadrant, rpn_head_cls, anchor_states)
        ...
        with tf.variable_scope('postprocess_detctions'):
            boxes, scores, category, boxes_head, boxes_angle = postprocess_detctions(rpn_bbox_pred=rpn_box_pred,
                                                                                     rpn_cls_prob=rpn_cls_prob,
                                                                                     rpn_angle_prob=tf.sigmoid(
                                                                                         rpn_angle_cls),
                                                                                     rpn_head_prob=tf.sigmoid(
                                                                                         rpn_head_cls),
                                                                                     anchors=anchors,
                                                                                     is_training=self.is_training)
