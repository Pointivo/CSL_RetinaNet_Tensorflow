...
def only_draw_boxes(img_batch, boxes, method, head=None):
    ...
    img_tensor_with_boxes = tf.py_func(draw_box_in_img.draw_boxes_with_label_and_scores,
                                       inp=[img_tensor, boxes, labels, scores, method, head],
                                       Tout=tf.uint8)
...
def draw_boxes_with_scores(img_batch, boxes, scores, method, head):
...
def draw_boxes_with_categories(img_batch, boxes, labels, method, head=None):
...
def draw_boxes_with_categories_and_scores(img_batch, boxes, labels, scores, method, head=None):
